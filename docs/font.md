...
Alternatively, fonts can be inlined directly into the CSS files by setting the
environment variable `BUNDLE_FONTS=true` before running `yarn build`.
This increases the size of the CSS file and may slow down page loading,
but may be necessary in certain contexts; providing paths to font files is
